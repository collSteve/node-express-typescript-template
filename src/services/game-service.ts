...
export class GameService implements IGameServiceDynamic {
    ...
    public async createGameForUser(creatorId:string, gameType:GameType, currentMove=true, maxUserCount?:number) {
        ...
        if (!NeededGamePlayerClass) throw new InvalidGameTypeError("GameType does not exist or it does not has a according game type.");
    ...
    public async joinUserToGame(userId:string, gameType:GameType):Promise<{gameStatus:GameStatus<any>, playerStatus:PlayerStatus<any>}> {
    ...
    private createGameForPlayer<GameInfoType, PlayerGameInfoType>(creatorPlayer:PlayerModel<PlayerGameInfoType>,
        gameClass:GameClass<GameInfoType, PlayerGameInfoType>, gameType:GameType, currentMove=true, maxUserCount?:number) {
