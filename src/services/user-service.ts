...
import OperationNotValidOnTempUserError from '../errors/operation-not-valid-on-temp-user-error';
...
export default class UserService {
	...
	public createTempUser(): void {
		...
		if (this.users.has(newUserId)) {
			throw new UserAlreadyExistsError(
				UserService.USER_ALREADY_EXISTS_MSG
			);
		}
	...
	public createPermanentUser(
		...
		const newUser: User = new PermanentUser(
			newUserId,
			userName,
			email,
			password
	...
	public deleteUser(userId: string): void {
		if (!this.users.has(userId)) {
			throw new UserDoesNotExistError(
				UserService.USER_DOES_NOT_EXIST_MSG
			);
		}

		// remove user from all games that he/she is associated with:
		const userGames: string[] | undefined = this.users
			.get(userId)
			?.getAllGameAssociations();
	...
	public retrieveGameUsers(gameId: string): User[] {
		return [...this.users.values()].filter((user: User) =>
			user.hasGame(gameId)
		);
	}

	/**
	 * Updates specified user's password if the user is a permanent user
	 * @param userId - the user id of user to update
	 * @param password - the new password
	 * @throws {UserDoesNotExistError | OperationNotValidOnTempUserError}
	 * @returns {User} the updated user
	 */
	public updateUserPassword(userId: string, password: string) {
		const ERROR_MESSAGE: string = `Cannot update password on temporary user`;
		const user: User = this.findUser(userId);
		if (!(user instanceof PermanentUser)) {
			throw new OperationNotValidOnTempUserError(ERROR_MESSAGE);
		}
	...
	public getUserById(userId: string) {
		const ERROR_MESSAGE = `User with Id ${userId} does not exist.`;
	...
	public removeGameAssociation(userId: string, gameId: string) {
		const ERROR_MESSAGE = `User with Id ${userId} does not exist.`;
		const user = this.users.get(userId);
		if (!user) {
			throw new UserDoesNotExistError(ERROR_MESSAGE);
